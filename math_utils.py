"""Mathematical and statistical utility functions with input validation and error handling."""
...
def fibonacci(n: int) -> int:
    ...
    Args: n - A non-negative integer index
    Returns: The nth Fibonacci number
    Raises: TypeError if n is not an integer, ValueError if n is negative"""
    if not isinstance(n, int) or isinstance(n, bool):
        raise TypeError(f"fibonacci() argument must be an integer, not {type(n).__name__}")
    if n < 0: raise ValueError("fibonacci() not defined for negative values")
    if n == 0: return 0
    if n == 1: return 1
    prev, curr = 0, 1
...
def gcd(a: int, b: int) -> int:
    ...
    Raises: TypeError if a or b is not an integer, ValueError if both are zero"""
